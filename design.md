...
Причины выбора `Vue 3 + Vite`:

- удобно строить отзывчивый streaming UI;
- зрелая экосистема для реактивного клиентского состояния, composables и component/e2e-тестов;
...
1. **Presentation Layer (`Vue 3 + Vite`)**
   ...
   - использует `Pinia` для реактивного стора и узкие компонентные границы (`computed`, `shallowRef`, `v-memo` где уместно) для высокой производительности стриминга токенов;
   - использует `Vue Transition` и CSS-анимации для плавных переходов: transition центрального экрана в 4-колоночный режим, появление колонок, индикаторы загрузки.
...
`Browser -> Vue UI -> Laravel API -> OpenRouter API`

Дополнительно:

- `Vue UI <- SSE stream <- Laravel API <- stream <- OpenRouter API`
...
## 2.1. Frontend-модули (`Vue 3 + Vite`)
...
- подписка компонентов на срезы состояния через `storeToRefs()` и доступ к данным колонки по `columnId` без перерендера всей страницы.
...
- Markdown-рендеринг ответов ассистента через `markdown-it`;
- минимизация лишних перерисовок через стабильные props, `computed` и `v-memo` там, где это оправдано;
...
Предлагаемая архитектура строится вокруг `Laravel` как доменного оркестратора и `Vue 3 + Vite` как streaming UI-клиента.
...
- плавные UI-анимации через `Vue Transition` и CSS;
